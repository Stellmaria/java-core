...
public class Symbol {
    ...
    public String getSymbolType(final char symbol) {
        ...
        if (symbol >= minDigit && symbol <= maxDigit) {
            return digit;
        } else if (symbol >= minLatin && symbol <= maxLatin) {
            return latin;
        } else if (symbol >= minLatinA && symbol <= maxLatinZ) {
            return latin;
        } else if (symbol >= minCyrillic && symbol <= maxCyrillic) {
            return cyrillic;
        } else if (symbol >= minCyrillicA && symbol <= maxCyrillicZ) {
            return cyrillic;
        } else {
            return undefined;
        }
