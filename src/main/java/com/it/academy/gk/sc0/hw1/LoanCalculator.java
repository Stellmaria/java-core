...
public class LoanCalculator {
     ...
     */
    public LoanCalculator(double loanAmount, int years, double interestRate) {
     ...
     * It first calculates the monthly interest rate by dividing the annual interest rate by the number
     * of months in a year and by 100 to convert from percentage. Then it calculates the total number
     * of months for the loan and uses these values to calculate and return the monthly payment.
     *
     * @return the monthly payment for the loan
     */
    public double calculateMonthlyPayment() {
        var monthlyInterestRate = interestRate / MONTHS_IN_YEAR / PERCENT;
        var months = years * MONTHS_IN_YEAR;
        var monthlyPayment = loanAmount * monthlyInterestRate / (1 - Math.pow(1 + monthlyInterestRate, -months));

        return Math.round(monthlyPayment * PERCENT) / (double) PERCENT;
    }

    /**
     * Calculates the total payment for the loan.
     * <p>
     * This method calculates and returns the total payment by multiplying the monthly payment
     * by the total number of months for the loan.
     *
