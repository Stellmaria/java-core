...
public class MultiplicationTable {
     ...
     * <p>
     * The printMultiplicationTable method generates a string representation of a multiplication table
     * for the given multiplier and number of rows. Each row of the table shows the result of multiplying
     * the row number by the multiplier. The rows are separated by newline characters.
     * The method uses an IntStream to generate a stream of integers from 1 to row, inclusive.
     * For each integer in the stream, it concatenates it with the MULTIPLICATION_SYMBOL, multiplier,
     * EQUALS_SYMBOL, and the result of multiplying the integer by multiplier. It then appends
     * a NEWLINE character to create a new row in the table. Finally, it collects all rows into
