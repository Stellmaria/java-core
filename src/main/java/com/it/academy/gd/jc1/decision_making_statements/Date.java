...
import com.it.academy.md.jc1.decision_making_statements.LeapYear;
...
public class Date {
    ...
    public String getNextDay(int day, int month, int year) {
        final boolean leap = leapYear.isLeapYear(year);
        ...
        if ((day == 28 || day == 29) &&
            (month == 2 && leap)) {
            getNextDay(month, year, stringBuilder);
        } else if ((day == 31) &&
     ...
     */
    private void getNextDay(int month, int year, final @NotNull StringBuilder stringBuilder) {
        final int day = 1;
        month++;
